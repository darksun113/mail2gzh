...
- 🤖 实现 GPT-5 翻译功能，支持智能邮件翻译
- 🖼️ 添加图片处理服务，自动下载并上传图片到微信公众号
- 📝 新增批量翻译 API 端点
- 🖼️ 添加图片处理 API 端点
- 👁️ 新增微信内容预览功能
- 📏 实现内容长度验证和智能截断
...
- 🔐 实现控制台二维码 OAuth 2.0 授权流程
- 🖥️ 支持在无图形界面的服务器环境中进行 Gmail API 授权
- 📱 添加二维码生成和显示功能

### 变更

- 🔄 将 Gmail API 认证方式从服务账号改回 OAuth 2.0 用户授权
- 📚 更新相关文档和配置说明
- 🎯 优化授权流程的用户体验

### 技术改进

- 📦 集成 `qrcode[pil]` 库用于生成二维码
- 🔧 更新 `google-auth-oauthlib` 和 `google-auth-httplib2` 依赖
- 📝 改进错误处理和日志记录
...
- 🔐 支持 Gmail 服务账号认证
- 📖 添加服务账号配置指南 (GMAIL_SERVICE_ACCOUNT_SETUP.md)
- 🌐 支持域范围委派访问
- 🖥️ 优化纯命令行环境部署

### 变更

- 🔄 将 Gmail API 认证从 OAuth 2.0 用户授权改为服务账号
- ⚙️ 更新环境变量配置，移除 token.json 依赖
- 📦 简化依赖管理，移除 google-auth-oauthlib
- 📚 更新文档和配置说明
